...
public class AddAlterMemoDialog extends DialogFragment {
	...
	public static AddAlterMemoDialog getInstance(SCCMainActivity act){
		AddAlterMemoDialog instance = new AddAlterMemoDialog();
	...
	public static AddAlterMemoDialog getInstance(SCCMainActivity act, long eventTime){
		AddAlterMemoDialog.eventTime = eventTime;
		existingEventTime = eventTime;
		activity = act;
				...
				if(existingEventHasToBeRemoved){//do it
					...
					if(datumId != null && !PersonalNetwork.VALUE_NOT_ASSIGNED.equals(datumId)){
						...
						history.setAttributeValueAt(TimeInterval.getTimePoint(existingEventTime), 
								PersonalNetwork.getAlterMemosAttributeName(), 
								Alter.getInstance(selectedAlter), PersonalNetwork.VALUE_NOT_ASSIGNED);
					}
				}
				history.
