...
import net.egosmart.scc.gui.util.AlterAlterAttributeDescriptionListBaseAdapter;
...
/**
 * Dialog that shows all ego alter attributes for which the selected alter has a defined value.
 * 
 * The values of these attributes can be changed. The user might also select from the declared 
 * attributes for which the selected alter does not have a defined value and the user might create a
 * totally new ego alter attribute.
 * 
 * @author juergen
 *
 */
public class EditAlterAlterAttributesDialog extends DialogFragment {

	private static SCCMainActivity activity;

	public static EditAlterAlterAttributesDialog getInstance(SCCMainActivity act){
		activity = act;
		return new EditAlterAlterAttributesDialog();
	}
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		headline.setText(selectedAlter + " | " + selectedSecondAlter);
		//attribute names
		ArrayList<String> attrNames = new ArrayList<String>(network.getAttributeNames(
				PersonalNetwork.DOMAIN_ALTER_ALTER));
		//generate those pairs of attribute names times direction that make sense
		ArrayList<Pair<String, String>> attrNamesDirectionPairs = new ArrayList<Pair<String,String>>();
		for(String attrName : attrNames){
			String dir = network.getAttributeDirectionType(PersonalNetwork.DOMAIN_ALTER_ALTER, attrName);
			if(PersonalNetwork.DYAD_DIRECTION_SYMMETRIC.equals(dir) || 
					PersonalNetwork.DYAD_DIRECTION_ASYMMETRIC.equals(dir)){
				attrNamesDirectionPairs.add(
						new Pair<String, String>(attrName, PersonalNetwork.DYAD_DIRECTION_OUT));
			}
			if(PersonalNetwork.DYAD_DIRECTION_ASYMMETRIC.equals(dir)){
				attrNamesDirectionPairs.add(
						new Pair<String, String>(attrName, PersonalNetwork.DYAD_DIRECTION_IN));
			}
		}
		//mapping attribute names times direction to value fields (which are either edittexts or spinners)
		final HashMap<Pair<String,String>,EditText> attrNameDirection2text = new HashMap<Pair<String,String>, EditText>();
		final HashMap<Pair<String,String>,Spinner> attrNameDirection2spinner = new HashMap<Pair<String,String>, Spinner>();
		//holds the rows of those attributes for which the alter has a value
		final TableLayout attribsTable = (TableLayout) contentViewGroup.
				findViewById(R.id.edit_attributes_dialog_table);
		//mapping from attribute names times direction for which the alter has no value to table rows (can be added in the GUI) 
		final LinkedHashMap<Pair<String,String>, TableRow> otherAttributes = new LinkedHashMap<Pair<String,String>, TableRow>();
		//iterate over all existing alter alter attributes and the in and out direction; 
		//for each of them: if the value is set for this alter in this direction, 
		//create a row where the value can be changed; if not, create a row that is stored in otherAttributes
		for(final Pair<String, String> nameDirection : attrNamesDirectionPairs){
			String value;
			if(nameDirection.second.equals(PersonalNetwork.DYAD_DIRECTION_OUT))
				value = network.getAttributeValueAt(System.currentTimeMillis(),
						nameDirection.first, 
			...
			TableRow row = createRow(network, attrNameDirection2text,
					attrNameDirection2spinner, attribsTable, otherAttributes,
					nameDirection, value);
			...
			if(!value.equals(PersonalNetwork.VALUE_NOT_ASSIGNED))
				...
				otherAttributes.put(nameDirection, row);
		}
			...
			public void onClick(View arg0) {
				new DialogFragment(){
					public Dialog onCreateDialog(Bundle savedInstanceState) {
						...
						//the dialog content view shows a headline, a list of alter-alter attributes that are 
						//declared in the network but have no value for this alter-alter pair, and a button to 
						//create a new alter attribute
						LinearLayout contentView = (LinearLayout) LayoutInflater.from(activity).
								inflate(R.layout.add_new_attribute_4ego_alter_view, null);
						TextView headline = (TextView) contentView.findViewById(R.id.add_new_attribute_4ego_alter_headline);
						headline.setText(selectedAlter + " | " + selectedSecondAlter);
						ListView attrList = (ListView) contentView.findViewById(R.id.add_new_attribute_4ego_alter_list);
						ArrayList<Pair<Pair<String, String>,String>> attrNameDirDescList = new ArrayList<Pair<Pair<String, String>,String>>();
						for(Pair<String,String> aNameDir : otherAttributes.keySet()){
							attrNameDirDescList.
							add(new Pair<Pair<String,String>, String>(aNameDir, 
									network.getAttributeDescription(PersonalNetwork.DOMAIN_ALTER_ALTER,
											aNameDir.first)));
						}
						AlterAlterAttributeDescriptionListBaseAdapter attrNameDirDescListAdapter = 
								new AlterAlterAttributeDescriptionListBaseAdapter(activity, attrNameDirDescList);
						attrList.setAdapter(attrNameDirDescListAdapter);
						attrList.setOnItemClickListener(new OnItemClickListener() {

							public void onItemClick(AdapterView<?> parent, View v, int position,
									long id) {
								Pair<String,String> a = ((Pair<Pair<String,String>,String>) parent
						...
						//alter alter attribute can be declared for the network 
						//and then a value can be selected for this alter
						Button createNewAttr = (Button) contentView.
								findViewById(R.id.create_new_attribute_from_edit_ego_alter_dialog);
							...
							public void onClick(View arg0) {
								new DialogFragment(){
									public Dialog onCreateDialog(Bundle savedInstanceState) {
										...
										createAttrDialogBuilder.setTitle(R.string.create_new_alter_alter_label_title);
										ViewGroup createAttrContentView = (ViewGroup) LayoutInflater.from(activity).
												inflate(R.layout.create_new_ego_alter_attribute_from_dialog_view, null);
										final EditText attrNameEditText = (EditText) createAttrContentView.
												findViewById(R.id.new_ego_alter_attribute_name_edit_text);
										final EditText attrDescEditText = (EditText) createAttrContentView.
												findViewById(R.id.new_ego_alter_attribute_desc_edit_text);
										...
										final LinearLayout newAttributeChoiceRow = (LinearLayout) createAttrContentView.
												findViewById(R.id.new_ego_alter_attribute_choice_row);
										final ListView newAttributeChoicesList = (ListView) createAttrContentView.
												findViewById(R.id.new_ego_alter_attribute_choices_list);
										final EditText newAttributeChoiceEditText = (EditText) createAttrContentView.
												findViewById(R.id.new_ego_alter_attribute_choice_edit_text);
										...
										Button addAttributeChoiceButton = (Button) createAttrContentView.
												findViewById(R.id.add_new_ego_alter_attribute_choice_button);
										...
										final Spinner attrDirectionSpinner = (Spinner) createAttrContentView
												.findViewById(R.id.new_ego_alter_attribute_direction_spinner);
										String[] dirTypes = {PersonalNetwork.DYAD_DIRECTION_SYMMETRIC,
										...
										final Spinner attrTypeSpinner = (Spinner) createAttrContentView
												.findViewById(R.id.new_ego_alter_attribute_type_spinner);
										...
										createAttrDialogBuilder.setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
											...
											public void onClick(DialogInterface dialog, int which) {
												...
												final String attrDir = attrDirectionSpinner.getSelectedItem().toString();
												String attrDesc = attrDescEditText.getText().toString().trim();
												if(attrName.length() > 0 && 
														!attrName.startsWith(PersonalNetwork.ATTRIBUTE_PREFIX_EGOSMART)){
													...
													if(attrTypeSpinner.getSelectedItemPosition() == PersonalNetwork.ATTRIB_TYPE_FINITE_CHOICE){
														...
														network.setAttributeChoices(PersonalNetwork.DOMAIN_ALTER_ALTER, attrName, choicesSet);
													}
													if(PersonalNetwork.DYAD_DIRECTION_ASYMMETRIC.equals(attrDir) ||
															PersonalNetwork.DYAD_DIRECTION_SYMMETRIC.equals(attrDir)){
														TableRow row = createRow(network, 
																attrNameDirection2text, 
																attrNameDirection2spinner, 
																attribsTable, 
																otherAttributes, 
																new Pair<String,String>(attrName,PersonalNetwork.DYAD_DIRECTION_OUT), 
												...
												} else {//attribute name is not valid for a user defined-attribute
		...
		builder.setPositiveButton(R.string.edit_attrib_ok_button_text, new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int which) {
				for(Pair<String,String> attrNameDir : attrNameDirection2text.keySet()){
					String value = attrNameDirection2text.get(attrNameDir).getText().toString().trim();
					if(attrNameDir.second.equals(PersonalNetwork.DYAD_DIRECTION_OUT))
				...
				for(Pair<String,String> attrNameDir : attrNameDirection2spinner.keySet()){
					Spinner spinner = attrNameDirection2spinner.get(attrNameDir);
	...
	private TableRow createRow(
			final PersonalNetwork network,
			final HashMap<Pair<String, String>, EditText> attrNameDirection2text,
			final HashMap<Pair<String, String>, Spinner> attrNameDirection2spinner,
			final TableLayout attribsTable,
			final LinkedHashMap<Pair<String, String>, TableRow> otherAttributes,
			final Pair<String, String> nameDirection, String value) {
		final TableRow row = (TableRow) LayoutInflater.from(activity).
				inflate(R.layout.edit_ego_alter_attributes_table_attr_row, null);
		TextView name = (TextView) row.findViewById(R.id.edit_ego_alter_attributes_dialog_attr_name);
		name.setText(nameDirection.first);
		ImageView directionImage = (ImageView) row.findViewById(R.id.edit_ego_alter_attributes_table_image_direction);
		if(nameDirection.second.equals(PersonalNetwork.DYAD_DIRECTION_OUT) &&
				!network.getAttributeDirectionType(PersonalNetwork.DOMAIN_ALTER_ALTER, nameDirection.first).
				equals(PersonalNetwork.DYAD_DIRECTION_SYMMETRIC)){
			directionImage.setImageResource(R.drawable.ic_info_attr_ego2alter);
		}
		if(nameDirection.second.equals(PersonalNetwork.DYAD_DIRECTION_IN)){
			directionImage.setImageResource(R.drawable.ic_info_attr_alter2ego);
		}
		final String empty = "";
		FrameLayout valueContainer = (FrameLayout) row.
				findViewById(R.id.edit_ego_alter_attributes_dialog_value_container);
		final ImageButton removeValueButton = (ImageButton) row.
				findViewById(R.id.remove_ego_alter_attribute_value_button);
		int attrType = network.getAttributeValueType(PersonalNetwork.DOMAIN_ALTER_ALTER, nameDirection.first);
		...
		if(attrType != PersonalNetwork.ATTRIB_TYPE_FINITE_CHOICE){
			...
			attrNameDirection2text.put(nameDirection, valueEditText);
			...
			HashSet<String> choices = network.getAttributeChoices(
					PersonalNetwork.DOMAIN_ALTER_ALTER, nameDirection.first);
			...
			attrNameDirection2spinner.put(nameDirection, spinner);
