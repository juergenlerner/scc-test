 ...
 *
 */
public class SelectInputGraphMLFileDialog extends DialogFragment {

	protected static final CharSequence GRAPHML_FILE_TYPE = ".graphml";
	private static SCCMainActivity activity;
	private static File path;
	private static SelectInputGraphMLFileDialog instance;

	public static SelectInputGraphMLFileDialog getInstance(SCCMainActivity act){
		activity = act;
		path = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS);
		instance = new SelectInputGraphMLFileDialog();
		return instance;
	}

	public static SelectInputGraphMLFileDialog getInstance(SCCMainActivity act, File directory){
		activity = act;
		path = directory;
		instance = new SelectInputGraphMLFileDialog();
		return instance;
		...
		fileListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View v, int position,
				...
				if(selectedFile.equals(moveUp)){
					...
					instance.dismiss();
					DialogFragment dialog = SelectInputGraphMLFileDialog.getInstance(activity, parentDir);
					...
					if(file.isDirectory()){
						instance.dismiss();
						DialogFragment dialog = SelectInputGraphMLFileDialog.getInstance(activity, file);
						dialog.show(activity.getSupportFragmentManager(), 
								SCCMainActivity.SELECT_OUTPUT_GRAPHML_FILE_DIALOG_TAG);							
					} else if(file.isFile()){
		...
		builder.setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if(graphmlFileName != null && graphmlFileName.length() > 0){
					...
					activity.importFromGraphMLFile(file);
					instance.dismiss();
				}
			}
		});
