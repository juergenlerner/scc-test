...
public class SelectHistoryOutputGraphMLFileDialog extends DialogFragment {
	...
	public static SelectHistoryOutputGraphMLFileDialog getInstance(SCCMainActivity act){
		activity = act;
	...
	public static SelectHistoryOutputGraphMLFileDialog getInstance(SCCMainActivity act, File directory){
		activity = act;
		path = directory;
		instance = new SelectHistoryOutputGraphMLFileDialog();
		return instance;
		...
		fileListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View v, int position,
				...
				if(selectedFile.equals(moveUp)){
					...
					instance.dismiss();
					DialogFragment dialog = SelectHistoryOutputGraphMLFileDialog.getInstance(activity, parentDir);
					...
					if(file.isDirectory()){
						instance.dismiss();
						DialogFragment dialog = SelectHistoryOutputGraphMLFileDialog.getInstance(activity, file);
						dialog.show(activity.getSupportFragmentManager(), 
								SCCMainActivity.SELECT_OUTPUT_GRAPHML_FILE_DIALOG_TAG);							
					} else if(file.isFile()){
		...
		builder.setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if(graphmlFileName != null && graphmlFileName.length() > 0){
					...
					activity.exportHistoryToGraphMLFile(file);
					instance.dismiss();
				}
			}
		});
