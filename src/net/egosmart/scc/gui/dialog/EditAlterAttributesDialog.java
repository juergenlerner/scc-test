...
/**
 * Dialog that shows all alter attributes for which the selected alter has a defined value.
 * 
 ...
 * totally new alter attribute.
 * 
 * @author juergen
 *
 */
public class EditAlterAttributesDialog extends DialogFragment {

	private static SCCMainActivity activity;

	public static EditAlterAttributesDialog getInstance(SCCMainActivity act){
		activity = act;
		return new EditAlterAttributesDialog();
	}
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		String title = getActivity().getString(R.string.edit_alter_attrib_dialog_title);
		title = title + " " + selectedAlter; // title is not set as title but as headline
		TextView headline = (TextView) contentViewGroup.findViewById(R.id.edit_attributes_dialog_headline);
		headline.setText(title);
		HashSet<String> allAttrNames = network.getAttributeNames(PersonalNetwork.DOMAIN_ALTER); 
		...
		//mapping from attribute names for which the alter has no value to table rows (can be added in the GUI) 
		final LinkedHashMap<String, TableRow> otherAttributes = new LinkedHashMap<String, TableRow>();
		//iterate over all existing alter attributes; for each of them: if the value is set for this 
		//alter, create a row where the value can be changed; if not, create a row that is stored in otherAttributes
		for(final String attrName : allAttrNames){
			String value = network.getAttributeValueAt(System.currentTimeMillis(), 
					attrName, Alter.getInstance(selectedAlter));
			...
			int attrType = network.getAttributeValueType(PersonalNetwork.DOMAIN_ALTER, attrName);
			...
			if(attrType != PersonalNetwork.ATTRIB_TYPE_FINITE_CHOICE){
				...
				HashSet<String> choices = network.getAttributeChoices(PersonalNetwork.DOMAIN_ALTER, attrName);
				...
				new DialogFragment(){
					public Dialog onCreateDialog(Bundle savedInstanceState) {
						...
						LinearLayout contentView = (LinearLayout) LayoutInflater.from(activity).
								inflate(R.layout.add_new_attribute_4alter_view, null);
						...
						for(String aName : otherAttributes.keySet()){
							attrNameDescList.add(new Pair<String, String>(aName, network.
									getAttributeDescription(PersonalNetwork.DOMAIN_ALTER, aName)));
						}
						...
						//alter attribute can be declared for the network 
							...
							public void onClick(View arg0) {
								new DialogFragment(){
									public Dialog onCreateDialog(Bundle savedInstanceState) {
										...
										createAttrDialogBuilder.setTitle(R.string.create_new_alter_label_title);
											...
											public void onClick(DialogInterface dialog, int which) {
												...
												if(attrName.length() > 0 && !attrName.startsWith(PersonalNetwork.ATTRIBUTE_PREFIX_EGOSMART)){
													...
													if(attrTypeSpinner.getSelectedItemPosition() == PersonalNetwork.ATTRIB_TYPE_FINITE_CHOICE){
														...
														network.setAttributeChoices(
																PersonalNetwork.DOMAIN_ALTER, attrName, choicesSet);
													}
													...
													final ImageButton removeValueButton = (ImageButton) newlyCreatedRow.
															findViewById(R.id.remove_attribute_value_button);
													int attrType = network.getAttributeValueType(PersonalNetwork.DOMAIN_ALTER, 
															attrName);
													if(attrType != PersonalNetwork.ATTRIB_TYPE_FINITE_CHOICE){
														final EditText valueEditText = new EditText(activity);
														if(attrType == PersonalNetwork.ATTRIB_TYPE_NUMBER){
														...
														HashSet<String> choices = network.getAttributeChoices(
																PersonalNetwork.DOMAIN_ALTER, attrName);
														final ArrayList<String> items = new ArrayList<String>(choices);
														items.add(empty);
														ArrayAdapter<String> adapter = new ArrayAdapter<String>(activity, 
																android.R.layout.simple_spinner_item, items);
			...
			public void onClick(DialogInterface dialog, int which) {
				for(String attrName : attr2text.keySet()){
					...
					network.setAttributeValueAt(TimeInterval.getRightUnboundedFromNow(), 
							attrName, Alter.getInstance(selectedAlter), value);
				}
