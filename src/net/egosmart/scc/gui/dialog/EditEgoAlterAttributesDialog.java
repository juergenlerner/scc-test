...
public class EditEgoAlterAttributesDialog extends DialogFragment {
	...
	public static EditEgoAlterAttributesDialog getInstance(SCCMainActivity act){
		activity = act;
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		for(String attrName : attrNames){
			String dir = network.getAttributeDirectionType(PersonalNetwork.DOMAIN_EGO_ALTER, attrName);
			if(PersonalNetwork.DYAD_DIRECTION_OUT.equals(dir) || 
					PersonalNetwork.DYAD_DIRECTION_SYMMETRIC.equals(dir) || 
			...
			if(PersonalNetwork.DYAD_DIRECTION_IN.equals(dir) || 
					PersonalNetwork.DYAD_DIRECTION_ASYMMETRIC.equals(dir)){
				attrNamesDirectionPairs.add(
		...
		//iterate over all existing ego alter attributes and the in and out direction; 
		...
		for(final Pair<String, String> nameDirection : attrNamesDirectionPairs){
			String value = network.getAttributeValueAt(System.currentTimeMillis(),
					nameDirection.first, 
					EgoAlterDyad.getInstance(selectedAlter, 
					nameDirection.second));
			...
			public void onClick(View arg0) {
				new DialogFragment(){
					public Dialog onCreateDialog(Bundle savedInstanceState) {
						...
						//the dialog content view shows a headline, a list of alter attributes that are 
						//declared in the network but have no value for this alter, and a button to 
						...
						for(Pair<String,String> aNameDir : otherAttributes.keySet()){
							...
							add(new Pair<Pair<String,String>, String>(aNameDir, 
									network.getAttributeDescription(PersonalNetwork.DOMAIN_EGO_ALTER, aNameDir.first)));
						}
						EgoAlterAttributeDescriptionListBaseAdapter attrNameDirDescListAdapter = 
								new EgoAlterAttributeDescriptionListBaseAdapter(activity, attrNameDirDescList);
						...
						//ego alter attribute can be declared for the network 
							...
							public void onClick(View arg0) {
								new DialogFragment(){
									public Dialog onCreateDialog(Bundle savedInstanceState) {
										...
										createAttrDialogBuilder.setTitle(R.string.create_new_ego_alter_label_title);
										...
										createAttrDialogBuilder.setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
											...
											public void onClick(DialogInterface dialog, int which) {
												...
												if(attrName.length() > 0 && !attrName.startsWith(PersonalNetwork.ATTRIBUTE_PREFIX_EGOSMART)){
													network.addAttribute(PersonalNetwork.DOMAIN_EGO_ALTER, 
															attrName, attrDesc, 
													...
													if(attrTypeSpinner.getSelectedItemPosition() == PersonalNetwork.ATTRIB_TYPE_FINITE_CHOICE){
														...
														network.setAttributeChoices(PersonalNetwork.DOMAIN_EGO_ALTER, 
																attrName, choicesSet);
													}
													if(PersonalNetwork.DYAD_DIRECTION_ASYMMETRIC.equals(attrDir) ||
															PersonalNetwork.DYAD_DIRECTION_OUT.equals(attrDir) ||
													...
													if(PersonalNetwork.DYAD_DIRECTION_ASYMMETRIC.equals(attrDir) ||
															PersonalNetwork.DYAD_DIRECTION_IN.equals(attrDir)){
														TableRow row = createRow(network, 
																attrNameDirection2text, 
																attrNameDirection2spinner, 
																attribsTable, 
			...
			public void onClick(DialogInterface dialog, int which) {
				for(Pair<String,String> attrNameDir : attrNameDirection2text.keySet()){
					...
					network.setAttributeValueAt(TimeInterval.getRightUnboundedFromNow(),
							attrNameDir.first, EgoAlterDyad.getInstance(selectedAlter, attrNameDir.second), value);
				}
	...
	private TableRow createRow(
		...
		if(nameDirection.second.equals(PersonalNetwork.DYAD_DIRECTION_OUT) &&
				!network.getAttributeDirectionType(PersonalNetwork.DOMAIN_EGO_ALTER, nameDirection.first).
		...
		int attrType = network.getAttributeValueType(PersonalNetwork.DOMAIN_EGO_ALTER, nameDirection.first);
		...
		if(attrType != PersonalNetwork.ATTRIB_TYPE_FINITE_CHOICE){
			...
			HashSet<String> choices = network.getAttributeChoices(PersonalNetwork.DOMAIN_EGO_ALTER, nameDirection.first);
