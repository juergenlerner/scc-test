...
public class AddEgoAlterContactEventDialog extends DialogFragment {
	...
	public static AddEgoAlterContactEventDialog getInstance(SCCMainActivity act){
		AddEgoAlterContactEventDialog instance = new AddEgoAlterContactEventDialog();
	...
	public static AddEgoAlterContactEventDialog getInstance(SCCMainActivity act, long eventTime){
		AddEgoAlterContactEventDialog.eventTime = eventTime;
		existingEventTime = eventTime;
		activity = act;
			...
			public void onClick(DialogInterface dialog, int id) {
				...
				if(existingEventHasToBeRemoved){//do it
					String datumId = history.getAttributeDatumIDAt(existingEventTime, 
							...
							EgoAlterDyad.getOutwardInstance(selectedAlter));//TODO: check direction
				...
				history.setAttributeValueAt(TimeInterval.getTimePoint(eventTime), 
						PersonalNetwork.getEgoAlterContactEventAttributeName(), 
						EgoAlterDyad.getOutwardInstance(selectedAlter), 
						"value");//TODO: use value for title
				...
				if(text.length() > 0)
					history.setSecondaryAttributeValue(datumID, 
							PersonalNetwork.getSecondaryAttributeNameText(), 
							text);
				SCCProperties.getInstance(activity).setPropertyAlterDetailExpandAlterEvents(true);
